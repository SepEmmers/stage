...
try {
    ...
    if (isset($_POST['get_personeel_data'])) {
        // Query om alle personeelsleden met hun naam en e-mail op te halen
        $sql = "CALL GetActievePersoneelDetails()";
        ...
        if ($result) {
            ...
            $response['data'] = $personeelData;
            echo json_encode($response);
        } else {
            $response['error'] = "Fout bij het uitvoeren van de query: " . $conn->error;
            echo json_encode($response);
        }
    } elseif (isset($_POST['get_monthly_history_admin'])) {
        $year = $_POST['year'];
        $month = $_POST['month'];
        ...
        echo json_encode($response);
        $stmt->close();
    } elseif (isset($_POST['delete_work_hour']) && isset($_POST['werkuren_id'])) {
        $werkurenId = $_POST['werkuren_id'];

        $sql = "CALL usp_DeleteWerkuur(?)"; // Roep de stored procedure aan
        $stmt = $conn->prepare($sql);
        ...
        $stmt->bind_param("i", $werkurenId);

        if ($stmt->execute()) {
            $response['success'] = true;
            $response['message'] = 'Werkuur succesvol verwijderd.';
        } else {
            throw new Exception("Fout bij het uitvoeren van de query: " . $stmt->error);
        }
        echo json_encode($response);
        $stmt->close();
    } elseif (isset($_POST['add_work_hours'])) {
        // Ontvang de data van het formulier
        ...
        // Roep de stored procedure aan om de werkuren toe te voegen
        $sql = "CALL VoegNieuweWerkurenToe(?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
        ...
        // Bind de parameters
        $stmt->bind_param("isssii", $persoon_id, $datum, $start_tijd, $eind_tijd, $school_id, $job_id);

        // Voer de query uit
        if ($stmt->execute()) {
            $response['success'] = true;
            $response['message'] = 'Werkuren succesvol toegevoegd.';
        } else {
            $response['error'] = "Fout bij het uitvoeren van de query: " . $stmt->error;
        }
        echo json_encode($response);
        $stmt->close();
    } else {
