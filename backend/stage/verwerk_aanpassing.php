...
try {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $starttijd_uur = isset($_POST['starttijd']) ? $_POST['starttijd'] : null;
        ...
        if ($pk_werkuren && $personeel_fk && $starttijd_uur && $datum) {
            ...
            // Voorbereiden van de SQL query om de gegevens te updaten
            // Je kunt hier ook een stored procedure aanroepen als je dat wilt.
            $sql = "UPDATE werkuren SET
                    starttijd = :starttijd,
                    eindtijd = :eindtijd,
                    datum = :datum,
                    school_fk = :school_fk,
                    job_fk = :job_fk
                    WHERE pk_werkuren = :pk_werkuren AND personeel_fk = :personeel_fk";
            ...
            if ($stmt->execute()) {
                ...
                header("Location: adminstart.php?melding=succes"); // Vervang index.php door de juiste pagina
                exit();
            } else {
...
<body>
    <p><a href="adminstart.php">Terug naar overzicht</a></p> </body>
</html>